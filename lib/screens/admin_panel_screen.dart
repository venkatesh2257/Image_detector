...
import '../services/admin_dataset_service.dart';
...
class _AdminPanelScreenState extends State<AdminPanelScreen> {
  ...
  final _service = AdminDatasetService();
  ...
  Future<void> _loadData() async {
    ...
    final path = await _service.baseDirectoryPath();
    if (!mounted) {
      return;
    }
    setState(() {
  ...
  Future<void> _saveSample() async {
    ...
    if (image == null || label.isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('Image and primary label are required')),
      );
      return;
    }
  ...
  Future<void> _exportManifest() async {
    ...
    await _service.exportManifestForTraining();
    await _loadData();
    if (!mounted) {
      return;
    }
    ScaffoldMessenger.of(context).showSnackBar(
      const SnackBar(content: Text('Exported training manifest successfully')),
    );
  }
        ...
        child: Padding(
          ...
          child: Column(
            ...
            children: [
              ...
              Text(
                _basePath == null
                    ? 'Preparing dataset storage...'
                    : 'Dataset path: $_basePath',
      ...
      child: ListView(
        children: [
          ...
          TextField(
            ...
            decoration: _inputDecoration('Primary Label (e.g. car)'),
          ),
          ...
          TextField(
            ...
            maxLines: 3,
            decoration: _inputDecoration('Description / extra metadata'),
          ),
          const SizedBox(height: 12),
          FilledButton.icon(
            onPressed: _isBusy ? null : _saveSample,
            icon: const Icon(Icons.save_alt),
            label: const Text('Save sample to training dataset'),
          ),
          const SizedBox(height: 10),
          OutlinedButton.icon(
            onPressed: _isBusy ? null : _exportManifest,
            icon: const Icon(Icons.upload_file),
            label: const Text('Export manifest for automated training'),
          ),
          if (_isBusy) ...[
            const SizedBox(height: 12),
